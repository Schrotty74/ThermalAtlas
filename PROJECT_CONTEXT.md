...
- `Sources/ThermalView/SystemInformation.swift` – lokal gelesene, auf Mac-Modell, Chip, Kernzahlen, Speicher und macOS-Version beschränkte Systeminformationen.
...
- Das Thermometer oben rechts öffnet ein separates lokales Fenster mit Mac-Modell, Apple-Chip, CPU-/GPU-Kernzahl, Arbeitsspeicher, internem Speicher und macOS-Version. Diese Angaben werden nur beim Öffnen gelesen; Seriennummern, UUIDs und andere Hardware-Kennungen werden nicht abgefragt oder angezeigt.
...
- `MANUAL.md` und `MANUAL.de.md` sind gleichwertige öffentliche Handbücher. Die zugehörigen PDFs unter `Documentation/` verwenden ein dunkles, an die App angelehntes Farbdesign und füllen acht gestaltete A4-Seiten pro Sprache mit der aktuellen Kartenoberfläche, Systemkontext mit CPU-/GPU-Last und RAM, Systeminformationen, Menüleistenanzeige, lokalem Temperaturverlauf, Temperaturwarnungen, Export, allen vier Themes sowie den Menüs `Themes`, `Scan Refresh`, `Window Size`, `Visible Temperatures`, `Menu Bar Display`, `Temperature Alerts`, `Start at Login`, `Language`, `Export` mit Diagnosebericht und `Manuals`. Das englische Handbuch dokumentiert Englisch als Standard; das deutsche Handbuch übersetzt die englischen Menübegriffe nachvollziehbar. Ihre reguläre Veröffentlichung erfolgt nur im beauftragten Beta-/Final-Releaseablauf mit Git-Push.
- Nach einem ausdrücklich beauftragten Final-Build wird der Handbuchstand auf `main` gegen `beta` geprüft. Ein neuerer Beta-Handbuchstand bleibt bis zu einem späteren Final-Build ausschließlich auf `beta`. Sind die Final-Handbücher aktueller, werden nur die Handbuchquellen, PDFs, ihr Generator und dessen unmittelbar verwendete Bildressourcen nach `beta` übernommen und gepusht. Final-Quellcode, Version, Tag und Release bleiben dabei ausschließlich auf `main`.
- Alle Screenshots und sonstigen Bildressourcen, die in öffentlichen README-Dateien, Handbüchern, PDFs oder Releases erscheinen, werden vor jedem ausdrücklich beauftragten Beta- oder Final-Build auf lokale Laufwerksnamen, Benutzernamen, Pfade und andere private Angaben geprüft und erforderlichenfalls anonymisiert.
...
- Der Systeminfo-Dialog liest nur die dort sichtbaren Hardware- und Betriebssystemangaben; Seriennummern, UUIDs und andere Hardware-Kennungen sind ausgeschlossen.
...
- `swift test -c debug` lief am 4. September 2026 mit 24 erfolgreichen Tests durch, einschließlich der Sensorzuordnungen, des zeitlich begrenzten lokalen Prozessaufrufs sowie der Frischelogik für Verlauf und Warnungen. Die vier Themes Adaptiv, Liquid Glass, Aurora und Ember wurden am 26. August 2026 manuell in Hell- und Dunkelmodus sowie mit aktivierter Transparenzreduktion geprüft.
