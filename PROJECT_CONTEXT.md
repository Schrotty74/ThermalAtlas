...
- `README.md` und `README.de.md` sind gleichwertige öffentliche Dokumentationen: Sie zeigen beide die vier Theme-Screenshots sowie gleichwertige Installations-, Gatekeeper-, Datenschutz-, Status-, Lizenz- und Linkinformationen. Ihre öffentliche Tabelle führt nur die veröffentlichten Beta- und Final-Build-Kanäle; der rein lokale Dev-Kanal wird dort nicht aufgeführt. Auf `main` steht direkt oben ein sichtbarer Hinweis mit Link zum meist aktuelleren `beta`-Branch und einer kurzen GitHub-Umschaltanleitung. Der Changelog bleibt ausschließlich Englisch.
