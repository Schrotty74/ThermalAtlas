...
- Nach einem ausdrücklich beauftragten Final-Build wird der Handbuchstand auf `main` mit `beta` verglichen. Ein neuerer Beta-Handbuchstand bleibt bis zu einem späteren Final-Build ausschließlich auf `beta`. Sind die Final-Handbücher aktueller, werden ausschließlich `MANUAL.md`, `MANUAL.de.md`, die zugehörigen PDFs unter `Documentation/`, `Documentation/build_manuals.py` sowie dessen unmittelbar verwendete Bildressourcen nach `beta` übernommen und gepusht. Diese ausdrücklich erlaubte Dokumentationssynchronisierung überträgt weder Final-Quellcode noch Version, Tag oder Release nach `beta`.
- Alle Screenshots und sonstigen Bildressourcen, die in öffentlichen README-Dateien, Handbüchern, PDFs oder Releases veröffentlicht werden, sind vor jedem ausdrücklich beauftragten Beta- oder Final-Build auf lokale Laufwerksnamen, Benutzernamen, Pfade und andere private Angaben zu prüfen und bei Bedarf zu anonymisieren.
...
- Ein Auftrag, eine neue Beta oder ein neues Final zu bauen beziehungsweise zu erstellen, umfasst Commit, Push auf den zugehörigen Branch und GitHub-(Pre-)Release mit Downloads. Dafür nach dem Commit `Scripts/build-release-package.sh` mit `--publish` verwenden; einen Prüfhinweis selbst beheben und den Ablauf fortsetzen. Dev bleibt lokal.
